...
CREATE TABLE members (
    ...
    -- AI 활용 제한 (기본: 제한 없음)
    -- 가족 아카이브이므로 수집·활용은 기본 허용한다.
    -- 본인이 특정 범위를 원치 않는다고 밝힌 경우에만 기록한다.
    ai_optout    VARCHAR(20),              -- NULL(제한 없음) | 'voice'(음성 합성 제외)
                                           -- | 'persona'(대화형 제외) | 'all'(AI 활용 전체 제외)
    optout_note  TEXT                      -- 사유·경위 메모
);
...
가족이 가족의 기록을 모으는 시스템이다. 앨범을 정리하고 어르신의 이야기를 녹음하는 것과 같은 일이므로, **수집과 활용에 별도의 동의 절차를 두지 않는다.**

- **기본은 전체 허용.** 본인이 특정 범위를 원치 않는다고 밝힌 경우에만 `members.ai_optout`에 기록한다.
- 민감한 기록(유언·재산·건강 등)은 `local_only`로 외부 API 전송을 차단한다. 이는 동의 문제가 아니라 **정보 유출 방지**를 위한 조치다.
- 생성된 AI 음성·텍스트에는 **"AI 생성" 표시**를 남겨 실제 기록과 구분한다. 몇 년 뒤 "이게 진짜 목소리인가"를 판별할 수 없으면 아카이브 자체의 신뢰가 무너진다.
- 페르소나 AI는 **"그 사람의 대체물"이 아니라 "그 사람이 남긴 이야기에 접근하는 도구"**로 규정한다.
- 가족 외부로의 공유·공개는 별개 문제이므로 그때 판단한다.
